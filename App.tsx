...
const App: React.FC = () => {
  ...
  const handleDownload = async () => {
    ...
    setDownloadError(null);
    try {
      await downloadEpkAsPdf(epkOutput, token);
    } catch (err) {
      console.error(err);
      setDownloadError("Sorry, the download failed. The EPK couldn't be generated by the server.");
    } finally {
              ...
              <>
                <div className="mb-6">
                    <button 
                      ...
                      className="w-full font-orbitron flex items-center justify-center gap-3 text-lg font-bold bg-cyan-600 hover:bg-cyan-700 disabled:bg-cyan-900 disabled:text-gray-500 disabled:cursor-not-allowed text-white py-3 px-6 rounded-md transition-all duration-300 transform hover:scale-105"
                    >
                        <DownloadIcon className="w-6 h-6" />
                        {isDownloading ? 'Downloading...' : 'Download EPK as PDF'}
                    </button>
                    {downloadError && <p className="mt-3 text-center text-red-400 text-sm">{downloadError}</p>}
                </div>
